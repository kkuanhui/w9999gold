...
export const toolbarHtmlToObj = (html) => {
  const wordChildren = genChildrenArr(html)
  const res = wordChildren.map(p => { 
    const res = {
      ...p,
      children: p.children.map(span => {
        return reduceSpanLevel(span)
      })
    }
    res.children = _.flatten(res.children)
    return res
  })
  return res
}

const reduceSpanLevel = (spanObj) => {
  //html to object
  const process = (cur, parentStyle = {}) => {
    return cur.children.map(child => {
      if(child.dom === "#text"){
        ...
        return process(child, {...parentStyle, ...cur.style})  
      }
    })
  }

  return _.flattenDeep(process(spanObj))
}
...
// the function is generated by ChatGPT. EditFrame helper
...
const coalescence = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    if (i === 0) {
      ...
      candidate.style.bold !== currentObj.style.bold ||
      candidate.style.fontStyle !== currentObj.style.fontStyle ||
      candidate.style.textDecoration !== currentObj.style.textDecoration
    ) {
