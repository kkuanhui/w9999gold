...
const WordEditor = ({ onEditing }) => {
      ...
      itemObj.children.forEach((child) => {
        ...
        if (child.dom === "#text") {
          ...
          if (child.style.fontWeight !== "bold")
            protoStyle = { ...protoStyle, fontWeight: "normal" };
          if (child.style.fontStyle !== "italic")
            protoStyle = { ...protoStyle, fontStyle: "normal" };
          if (child.style.textDecoration !== "underline")
            protoStyle = { ...protoStyle, textDecoration: "none" };
          checkStyle(child);
        }
      });
    };
    ...
    while (preNode !== focusNode) {
      ...
      if (curNode.parentNode.nextSibling) {
        ...
        if (curNode.parentNode.parentNode.nextSibling.tagName !== "P") {
          ...
          curNode =
            curNode.parentNode.parentNode.nextSibling.firstChild.firstChild;
        }
      }
    }
    return queue;
  ...
  const handleClick = (e) => {
    ...
    const style = cssObjsInRange.reduce(
      (res, cur) => {
        if (cur.fontWeight !== "bold") {
          res.fontWeight = "normal";
        }
        if (cur.fontStyle !== "italic") {
          res.fontStyle = "normal";
        }
        if (cur.textDecoration !== "underline") {
          res.textDecoration = "none";
        }
        return res;
      },
      {
